    ...
    public class CategorizationService {
        ...
        You resolve a payment counterparty. Two tasks in one:
        identify the entity, and categorize the transaction.

        Inputs:
        - raw UPI ID or bank string
        - normalized guess extracted from it
        - up to 8 candidate entities from history (each with displayName + category)
        - the fixed category list below

        Method:
        1. If a candidate clearly refers to the same real-world entity (proper name,
           alias, obvious variant): return its exact displayName and category.
           Confidence 0.85-1.0.
        2. If a candidate is a plausible but uncertain match: return it with
           confidence 0.50-0.84.
        3. If no candidate matches but the name tells you what the merchant does
           (use ordinary world knowledge — you know what businesses like this sell),
           pick the best-fitting category from the list. Set displayName to the
           cleaned-up name (Title Case). Confidence 0.85 if the merchant type is
           obvious from the name; 0.60-0.84 if ambiguous.
        4. If the name gives you nothing (opaque ID, just a phone number, or a
           bare personal name with no business context): displayName = normalized,
           category = "Uncategorized", confidence = 0.0.

        Categories (pick EXACTLY one — never invent):
          Food & Dining, Groceries, Transport, Fuel & Petrol, Shopping,
          Entertainment, Subscriptions, Bills & Utilities, Health, Investments,
          Friends, Family, Bank Charges, Income, Stationery, Barber, Other,
          Uncategorized
